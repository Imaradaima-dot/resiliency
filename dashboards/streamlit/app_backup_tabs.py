...
with tab1:
    ...
    if not events_df.empty:
        events_df = events_df.sort_values("count", ascending=True)
        ...
        fig_events = px.bar(
            ...
            y="event_type",
            orientation="h",
            text=events_df["percentage"].map(lambda x: f"{x:.2f}%"),
            title="GitHub Events by Type",
        )
        fig_events.update_traces(marker_color=EVENT_COLOR, textposition="outside")
    ...
    if not health_df.empty:
        ...
        with c1:
            fig_latency = px.bar(
                ...
                text_auto=True,
                title="Region Latency by Health Status",
            )
        ...
        with c2:
            fig_lag = px.bar(
                ...
                color_discrete_map=STATUS_COLORS,
                text_auto=True,
                title="Estimated Replication Lag",
            )
